...
export default function ForgotPasswordPage() {
  ...
  return (
    <main className="min-h-[80vh] flex items-center justify-center bg-[#0a0a0a] p-6 relative overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_rgba(255,153,28,0.05),_transparent_70%)]"></div>
      ...
      <div className="w-full max-w-md bg-[#111] p-10 rounded-xl border border-white/5 relative z-10">
        ...
        {submitted ? (
          <div className="text-center animation-fadeIn">
            ...
            <Link href="/login" className="text-[#FF991C] text-sm font-bold uppercase tracking-widest hover:underline">
              ← Back to Login
            </Link>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <input
                ...
                className="w-full bg-black border border-white/10 px-4 py-3 text-white placeholder:text-white/20 focus:outline-none focus:border-[#FF991C] transition-colors"
              />
            </div>

            <button
              type="submit"
              className="w-full py-4 bg-white text-black font-bold uppercase tracking-widest text-sm hover:bg-[#FF991C] hover:text-white transition-all"
            >
              Send Reset Link
            </button>
